<template>
  <footer class="bg-gray-800">
    <div
      class="
        max-w-7xl
        border-l border-r border-dashed border-gray-700
        mx-auto
        px-4
      "
    >
      <div class="pt-5 pb-12 md:flex md:items-center md:justify-between">
        <div class="flex space-x-6 md:order-2">
          ...
          <a
            v-if="$config.social.gitlab"
            target="_blank"
            rel="noreferrer"
            :href="`https://gitlab.com/${$config.social.gitlab}`"
            class="text-gray-400 hover:text-gray-300"
          >
            <span class="sr-only">GitLab</span>
            <img src="images/gitlab-logo-500.svg" class="h-5 w-5" />
          </a>
          <a
        ...
        <p class="mt-8 text-sm leading-6 text-gray-400 md:mt-0 md:order-1"></p>
        <p
          class="mt-8 text-sm leading-6 text-gray-400 md:mt-0 md:order-1"
          v-if="false"
        >
          {{ $config.name }} • dev template by
...
export default {
  ...
  data() {
    return {
      ...
      mode: "dark",
      description:
        "Software Engineer who tinkers with side projects, every now n then.",
